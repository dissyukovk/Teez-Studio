const authService = {
  async login(username, password) {
    try {
      const response = await fetch('http://192.168.6.162:8000/api/token/', {
        method: 'POST',
        headers: {
...
async getUserData() {
  ...
  try {
    const response = await fetch('http://192.168.6.162:8000/api/user/', {
      method: 'GET',
      headers: {
...
async refreshToken() {
  ...
  try {
    const response = await fetch('http://192.168.6.162:8000/api/token/refresh/', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
