...
const AdminProducts = () => {
  ...
  const fetchProducts = async (page = 1) => {
    try {
      setLoading(true);
      const response = await axios.get(`http://192.168.6.15:8000/api/products/?page=${page}`, {
...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.post('http://192.168.6.15:8000/api/upload-batch/', { data: fileData }, {
      headers: getAuthHeaders(), // Добавляем заголовки аутентификации
    });
