...
import RequestModalSPhCheck from './RequestModalSPhCheck'; // Импортируем новый модальный компонент

const PhCheckRequests = ({ status }) => {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      const response = await requestService.getRequests({
        status: 4, // Фильтрация по статусу
...
export default PhCheckRequests;
