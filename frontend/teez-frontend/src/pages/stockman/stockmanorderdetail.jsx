...
import { Layout, Table, Descriptions, Typography, message, Spin, Button, Modal } from 'antd';
...
const StockmanOrderDetailPage = ({ darkMode, setDarkMode }) => {
  ...
  const [user, setUser] = useState(null);
    ...
    useEffect(() => {
      ...
      // Если storedUser пустой (или {}), user станет {}
      setUser(storedUser);
    }, []);
    ...
    useEffect(() => {
      if (!accessChecked) {
        // Если user ещё пустой (null или {}), пропускаем проверку,
        // чтобы дождаться, пока Sidebar обновит localStorage
        if (!user || Object.keys(user).length === 0) {
          return;
        }
  
        // Теперь у нас точно есть user. Проверяем groups
  ...
  // Экспорт в Excel с нумерацией строк
  ...
  // Функция печати с нумерацией строк
  const handlePrint = () => {
    if (!orderData) return;
