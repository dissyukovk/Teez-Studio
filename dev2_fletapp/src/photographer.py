...
def photographer_requests_page(token, page, content_container, user_data):
    ...
    def show_request_details(request_number):
        ...
        def start_shooting_handler(e):
            ...
            # Теперь у нас есть user_data в области видимости функции photographer_requests_page
            start_shooting(
                token,
                request_number,
                page,
                content_container,
                lambda: photographer_requests_page(token, page, content_container, user_data),
                user_data  # Передаём user_data сюда
            )
